...
dlo::OdomNode::OdomNode(ros::NodeHandle node_handle) : nh(node_handle)
{
  ...
  this->pub_loop_could = nh.advertise<sensor_msgs::PointCloud2>("/cloud_for_loop", 100);
  this->pub_odom = this->nh.advertise<nav_msgs::Odometry>("odom_for_loop", 1);
...
void dlo::OdomNode::publishPose()
{
  ...
  pcl::toROSMsg(*this->current_scan, laserCloudFullRes3Local);
  laserCloudFullRes3Local.header.stamp = this->scan_stamp;
  laserCloudFullRes3Local.header.frame_id = child_frame;
  pub_loop_could.publish(laserCloudFullRes3Local);
  // info：end
  ...
  this->pub_odom.publish(this->odom);
...
void dlo::OdomNode::icpCB(const sensor_msgs::PointCloud2ConstPtr &pc)
{
  ...
  // // info: 发布给后端的点云
  // 		sensor_msgs::PointCloud2 laserCloudFullRes3Local = *pc;
  // 		laserCloudFullRes3Local.header.frame_id = child_frame;
  // 		pub_loop_could.publish(laserCloudFullRes3Local);
  // // info：end
